...
my_api_key = "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"  # 在这里输入你的 API 密钥
...
# Note 对话需求大则令 raw = False
def get_response(system, context, myKey, raw=True):
...
def predict(chatbot, input_sentence, system, context, myKey):
    ...
    chatbot.append((input_sentence, message))
    return chatbot, context
...
def reduce_token(chatbot, system, context, myKey):
    ...
    statistics = (
        f'本次对话Tokens用量【{response["usage"]["completion_tokens"]+12+12+8} / 4096】'
    )
...
with gr.Blocks() as demo:
    ...
    newSystemPrompt = gr.Textbox(
        show_label=True,
        placeholder=f"在这里输入新的System Prompt...",
        label="更改 System prompt",
    ).style(container=True)
    systemPromptDisplay = gr.Textbox(
        show_label=True,
        value=initial_prompt,
        interactive=False,
        label="目前的 System prompt",
    ).style(container=True)
    with gr.Accordion(
        label="保存/加载对话历史记录(在文本框中输入文件名，点击“保存对话”按钮，历史记录文件会被存储到本地)", open=False
    ):
        with gr.Column():
            with gr.Row():
    ...
    txt.submit(
        predict,
        [chatbot, txt, systemPrompt, context, myKey],
        [chatbot, context],
        show_progress=True,
    )
    ...
    retryBtn.click(
        retry,
        [chatbot, systemPrompt, context, myKey],
        [chatbot, context],
        show_progress=True,
    )
    delLastBtn.click(
    ...
    uploadBtn.upload(
        load_chat_history,
        uploadBtn,
        [chatbot, systemPrompt, context, systemPromptDisplay],
        show_progress=True,
    )
    saveBtn.click(
        save_chat_history,
        [saveFileName, systemPrompt, context],
        None,
        show_progress=True,
    )
